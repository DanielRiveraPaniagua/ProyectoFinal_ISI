...
public class Graph {
   ...
   /**
    ...
    /**
     * Initializes a graph from the specified string, using the specified delimiter.
     *
     * @param str the string to convert to graph
     * @param delimiter the delimiter
     */
    public Graph(String str, String delimiter, String eol) {
        st = new ST<String, SET<String>>();
        String[] lines = str.split(eol);
        for (int i = 0; i < lines.length; i++) {
        	String[] names = lines[i].split(delimiter);
        	for (int j = 1; j < names.length; j++) {
                addEdge(names[0], names[j]);
            }
        }
    }
