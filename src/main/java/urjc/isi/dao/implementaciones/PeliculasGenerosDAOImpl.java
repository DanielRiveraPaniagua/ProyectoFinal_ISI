...
public class PeliculasGenerosDAOImpl extends RelacionesDAOImpl<Relaciones>{
  	...
  	public void createTable() throws SQLException{
		...
		statement.executeUpdate("create table peliculasgeneros (idpelicula text, genero text, PRIMARY KEY (idpelicula,genero))");
		c.commit();
  	}
  	@Override
	  ...
	  String sql = "INSERT INTO peliculasgeneros(idpelicula,genero) VALUES(?,?)";

  		try (PreparedStatement pstmt = c.prepareStatement(sql)) {
  			pstmt.setString(1, entity.getIdtabla1()); //Peliculas
  			pstmt.setString(2, entity.getIdtabla2()); //Genero
  			pstmt.executeUpdate();
