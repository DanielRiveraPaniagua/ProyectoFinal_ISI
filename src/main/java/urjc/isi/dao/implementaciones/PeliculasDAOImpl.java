...
public class PeliculasDAOImpl extends GenericDAOImpl<Peliculas> implements PeliculasDAO{
	...
	public List<Peliculas> selectPeliculasForAdultos(){
		...
		String sql = "SELECT titulo from peliculas  where calificacion::INTEGER = 1";
		try (PreparedStatement pstmt = c.prepareStatement(sql)) {
			ResultSet rs = pstmt.executeQuery();
			c.commit();
			while(rs.next()){
	...
	public List<Peliculas> selectPeliculasForNinos(){
		...
		String sql = "SELECT titulo from peliculas where calificacion::INTEGER = 0";
		try (PreparedStatement pstmt = c.prepareStatement(sql)) {
			ResultSet rs = pstmt.executeQuery();
			c.commit();
			while(rs.next()){
	...
	public List<Peliculas> selectCalificacionForPelicula(String name){
		...
		String sql = "SELECT calificacion::INTEGER from peliculas where titulo="+"'"+name+"'";
		try (PreparedStatement pstmt = c.prepareStatement(sql)) {
			ResultSet rs = pstmt.executeQuery();
			c.commit();
			while(rs.next()){
